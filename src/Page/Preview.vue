...
onMounted(async () => {
  listenId.value = await appWindow.listen<ImagePayload>("preview-img", ({event, payload}) => {
    console.log(payload)
...
<template>
  <v-row align-content="center" class="pa-2 bg-amber-accent-1 h-screen" no-gutters>
    ...
    <v-col align-self="center" class=" h-100">
      <v-card class="fill-height d-flex flex-column" rounded :loading="loading">
        ...
        <v-card-text  class="align-center pa-2" style="flex-grow: 1">
          <v-row id="preview-area" align="center" align-content="center" class="fill-height ma-0" justify="center" no-gutters>

            <v-img :src="objUrl" alt="Image" class=" fill-height align-center "
                   contain eager=""
            >
            </v-img>
          </v-row>
