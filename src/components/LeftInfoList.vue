<template>
  <v-container class="h-screen d-flex flex-column ma-0 pa-2">
    ...
    <v-card loading class="pa-2 h-100">
      <v-card-item
        class="h-100 overflow-y-auto overflow-x-hidden elevation-2"
...
const listInfo: ExpandItem[] = [
  {
    name: "A",
    desription: "F://A/b?12acacacacacacac",
    icon: "mdi-unity",
    childen: [
      {
        name: "B",
        desription: "Texture2D RGBA32 1245kiB",
        onClick: () => {},
        icon: "mdi-image",
      },
      {
...
const onClick = async () => {
  await dialog.message("Now Import Unity Bound file", {
    okLabel: "确定",
    type: "info",
    title: "Message",
  });
